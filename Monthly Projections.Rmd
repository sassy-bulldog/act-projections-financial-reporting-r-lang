...
proj.env$MonthlyProjections <- expand.grid(
  ...
  rename(
    ...
    ULAE_Perc = ULAE.NotInCedingCommission,
  ...
  mutate(
    ULAE_Perc = tidyr::replace_na(ULAE_Perc, 0),
    Expenses_Perc = tidyr::replace_na(Expenses_Perc, 0),
...
proj.env$table1_Earning_Pattern <- proj.env$MonthlyProjections %>%
  ...
  filter(WP_Month <= Expiration_Date) %>%
  rowwise() %>%
  mutate(
...
# Validate that all EP has been attributed
proj.env$total_ep_src <- sum(proj.env$table2_Earning_Pattern$EP_Monthly)
proj.env$total_ep_prj <- sum(proj.env$MonthlyProjections$EP_Monthly)
if (proj.env$total_ep_src != proj.env$total_ep_prj) {
  stop("Not all forecasted earned premium was allocated to the projection table!")
}

# Validate that the sum of projected earned premium reconciles to the written premium in treaty period
proj.env$EP_Check <- proj.env$MonthlyProjections %>%
  mutate(
    WP_InTreatyPeriod = dplyr::if_else(MONTH <= Expiration_Date, WP_Monthly, 0)
  ) %>%
  group_by(
    KEY
  ) %>%
  summarize(
    total_ep_monthly = sum(EP_Monthly + EP_TransferredToNextTreaty, na.rm = TRUE),
    total_wp_monthly = sum(WP_InTreatyPeriod, na.rm = TRUE)
  ) %>%
  mutate(
    Difference = abs(total_wp_monthly - total_ep_monthly),
    PercentDifference = Difference / ifelse(total_ep_monthly != 0, total_ep_monthly, NA)
  ) %>%
  filter(Difference > 1 | PercentDifference > 0.01)


# Moved the check to after we replace actual EP, because since we no longer calculate post
# Need another check earlier where the EP appended = original table and that subcalc has totals that match
if (nrow(proj.env$EP_Check) > 0) {
  stop(paste(
    "Total WP_Monthly and Total EP_Monthly are not equal for the following KEY(s):",
    paste(proj.env$EP_Check$KEY, proj.env$EP_Check$total_ep_monthly, proj.env$EP_Check$total_wp_monthly, proj.env$EP_Check$Difference, collapse = "\n")
  ))
}

proj.env$MonthlyProjections <- proj.env$MonthlyProjections %>%
  mutate(
    # Swap out actual earned premium where reported
    EP_Monthly = datareplace(EP_Monthly, EP_Monthly_Data),
...
proj.env$EP_Check <- proj.env$MonthlyProjections %>%
  ...
  summarize(
    total_ep_monthly = sum(EP_Monthly + EP_TransferredToNextTreaty - IUEPR_Monthly, na.rm = TRUE),
    total_wp_monthly = sum(WP_Monthly, na.rm = TRUE)
  ) %>%
  mutate(
    Difference = abs(total_wp_monthly - total_ep_monthly),
    PercentDifference = Difference / ifelse(total_ep_monthly != 0, total_ep_monthly, NA)
  ) %>%
  filter(PercentDifference > 0.01)
...
proj.env$MonthlyProjections <- proj.env$MonthlyProjections %>%
  rename(
    ...
    LLAE.Corridor.Attachment.LR. = LLAE.Corridor.Start,
    LLAE.Corridor.End.LR. = LLAE.Corridor.End,
    LLAE.Corridor...Retained. = LLAE.Corridor.CedentRetentionRate,
    Loss.Ratio.Cap. = LLAE.Cap,
    Minimum.Ceding.Commission. = CedingCommission.Minimum.Rate,
    Provisional.Ceding.Commission. = CedingCommission.Provisional.Rate,
    Maximum.Ceding.Commission. = CedingCommission.Maximum.Rate,
    Minimum.Ceding.Commission.LLAE.Ratio. = CedingCommission.Minimum.LLAE,
    Provisional.Ceding.Commission.LLAE.Ratio. = CedingCommission.Provisional.LLAE,
    Maximum.Ceding.Commission.LLAE.Ratio. = CedingCommission.Maximum.LLAE,
